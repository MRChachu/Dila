...
// 🟢 გლობალური ფუნქციები, რათა ყველა ბლოკმა დაინახოს ერთმანეთი
...
function processDamkaMove(roomId, playerId, from, to) {
                ...
                try {
                    ...
                    if (dbUser) {
                        ...
                        while (dbUser.xp >= levelThreshold) { 
                            dbUser.xp -= levelThreshold; dbUser.level += 1; levelThreshold = dbUser.level * 1000; 
                        }
...
function startGameLogic(roomId) {
    ...
    while (room.tableCards.length < 4) {
      ...
      if (c.rank === 'J') room.deck.push(c); else room.tableCards.push(c);
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('tryFindMatch', async ({ playerName, gameType, maxPlayers, isRanked }) => {
    ...
    let userAvatar = '😎'; let userVip = null; let userXp = 0;
    try { 
        const dbUser = await User.findOne({ username: playerName }); 
        if (dbUser) { userAvatar = dbUser.avatar || '😎'; userVip = dbUser.vipUntil; userXp = dbUser.xp || 0; } 
    } catch(e) {}

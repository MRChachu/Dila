...
const translations = {
  ka: {
    rules: "წესები", system: "სისტემა", about: "ჩვენს შესახებ", onlinePlatform: "ონლაინ პლატფორმა",
    heroTitle1: "ითამაშე ფურთი", heroTitle2: "პროფესიონალურად", heroDesc: "რეიტინგული მატჩები, უნიკალური მიღწევები და ტოპ ათეული. შეუერთდი საქართველოს საუკეთესო მოთამაშეებს.",
    ranked: "რეიტინგული", shop: "მაღაზია", login: "შესვლა", register: "რეგისტრაცია",
    ...
    wait: "დაელოდეთ...", continue: "გაგრძელება", changePass: "პაროლის შეცვლა", forgot: "დაგავიწყდა პაროლი?", back: "უკან", backMain: "მთავარზე დაბრუნება",
    errFields: "შეავსეთ ყველა ველი!", errPassMatch: "პაროლები არ ემთხვევა!", errPassRule: "პაროლი: მინ 6 სიმბოლო, 1 ასო, 1 ციფრი!", errServer: "სერვერთან კავშირი ვერ მოხერხდა", errData: "მონაცემები არასწორია",
    errUserRule: "სახელი: მინ 3 ლათინური ასო/ციფრი, Space-ის გარეშე!", errAgeRule: "რეგისტრაციისთვის აუცილებელია იყოთ მინიმუმ 16 წლის!",
    ...
    footerDesc: "ქართული დეველოპერული პროექტი. კლასიკური ბანქოს თამაშის თანამედროვე, სანდო და აზარტული ონლაინ სივრცე.", allRights: "ყველა უფლება დაცულია.",
  ...
  en: {
    rules: "Rules", system: "System", about: "About Us", onlinePlatform: "Online Platform",
    heroTitle1: "Play Phurti", heroTitle2: "Professionally", heroDesc: "Ranked matches, unique achievements, and top 10 leaderboards. Join the best players.",
    ranked: "Ranked", shop: "Shop", login: "Login", register: "Register",
    ...
    wait: "Please wait...", continue: "Continue", changePass: "Change Password", forgot: "Forgot password?", back: "Back", backMain: "Back to main",
    errFields: "Fill in all fields!", errPassMatch: "Passwords do not match!", errPassRule: "Password: min 6 chars, 1 letter, 1 number!", errServer: "Server connection failed", errData: "Incorrect details",
...
export default function Auth({ onAuthSuccess }) {
  ...
  const handleNextStep = async (e) => {
    ...
    try {
      ...
      if (res.ok) setRecoveryStep(2);
      else setError(data.message || t.errData);
    } catch (err) { setError(t.errServer); } 
  ...
  const handleSubmit = async (e) => {
    ...
    if (authMode === 'register') {
      ...
      if (username.length < 3 || !userRegex.test(username)) return setError(t.errUserRule);
      if (!dob) return setError(t.errFields);
      const birthDate = new Date(dob); const today = new Date();
      let age = today.getFullYear() - birthDate.getFullYear();
      const m = today.getMonth() - birthDate.getMonth();
      if (m < 0 || (m === 0 && today.getDate() < birthDate.getDate())) age--;
      if (age < 16) return setError(t.errAgeRule);
      if (!validatePassword(password)) return setError(t.errPassRule);
    }

    if (authMode === 'forgot' && password !== confirmPassword) return setError(t.errPassMatch);
    if (authMode === 'forgot' && !validatePassword(password)) return setError(t.errPassRule);

    setIsLoading(true);
    let endpoint = authMode === 'login' ? '/api/auth/login' : authMode === 'register' ? '/api/auth/register' : '/api/auth/reset-password';
    let payload = authMode === 'login' ? { username, password } : authMode === 'register' ? { username, dateOfBirth: dob, secretWord, password } : { username, dateOfBirth: dob, secretWord, newPassword: password };
        ...
        <section id="rules" className="py-10 border-t border-white/5 bg-stone-950/30">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[ { icon: Target, title: t.goal, desc: t.goalDesc }, { icon: BookOpen, title: t.scoring, desc: t.scoringDesc }, { icon: Shield, title: t.capture, desc: t.captureDesc } ].map((rule, i) => (
                <div key={i} className="bg-stone-900/40 p-4 rounded-2xl border border-white/5 flex gap-3 items-start">
                  <div className="w-8 h-8 bg-stone-950 rounded-lg flex items-center justify-center border border-white/5 text-yellow-500 shrink-0"><rule.icon size={16} /></div>
                  <div><h3 className="text-[11px] font-black text-stone-200 mb-1 uppercase">{rule.title}</h3><p className="text-[10px] text-stone-400 font-medium">{rule.desc}</p></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-stone-900/60 to-stone-950/40 p-6 rounded-[2rem] border border-white/5 space-y-5">
                <div className="flex items-center justify-between"><div><div className="flex items-center gap-1.5 text-yellow-500 mb-1"><Coins size={14}/> <span className="text-[9px] font-black uppercase tracking-widest">{t.economy}</span></div><h3 className="text-xl font-black text-stone-100 uppercase tracking-tight">{t.shopQuests}</h3></div></div>
                <p className="text-[11px] text-stone-400 font-medium leading-relaxed">{t.economyDesc}</p>
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {[ { icon: Target, title: t.dailyQuests, desc: t.dailyQuestsDesc }, { icon: Palette, title: t.designs, desc: t.designsDesc }, { icon: Store, title: t.premiumAvatar, desc: t.premiumAvatarDesc }, { icon: Crown, title: t.vipStatus, desc: t.vipStatusDesc } ].map((f, i) => (
                    <div key={i} className="bg-stone-950/50 p-3 rounded-xl border border-white/5"><f.icon size={14} className="text-yellow-500 mb-1.5" /><h4 className="text-[10px] font-black text-stone-200 uppercase">{f.title}</h4><p className="text-[9px] text-stone-500 mt-0.5">{f.desc}</p></div>
                  ))}
                </div>
              </div>

              <div className="bg-gradient-to-br from-stone-900/60 to-stone-950/40 p-6 rounded-[2rem] border border-white/5 space-y-5">
                <div><div className="flex items-center gap-1.5 text-yellow-500 mb-1"><Star size={14}/> <span className="text-[9px] font-black uppercase tracking-widest">Hardcore</span></div><h3 className="text-xl font-black text-stone-100 uppercase tracking-tight">{t.achievementsSys}</h3></div>
                <p className="text-[11px] text-stone-400 font-medium leading-relaxed">{t.achievementsDesc}</p>
                <div className="space-y-2 pt-2">
                  {[ { icon: '🛡️', title: t.achVet, desc: t.achVetDesc }, { icon: '💎', title: t.ach10D, desc: t.ach10DDesc }, { icon: '♣️', title: t.ach2C, desc: t.ach2CDesc }, { icon: '🧹', title: t.achSweep, desc: t.achSweepDesc } ].map((ach, i) => (
                    <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl bg-stone-950/50 border border-white/5"><span className="text-xl drop-shadow-md bg-stone-900 p-1.5 rounded-lg border border-white/5">{ach.icon}</span><div><h4 className="text-[10px] font-black text-stone-200 uppercase">{ach.title}</h4><p className="text-[9px] text-stone-500">{ach.desc}</p></div></div>
                  ))}
                </div>
              </div>
            </div>

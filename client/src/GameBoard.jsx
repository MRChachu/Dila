...
export default function GameBoard({ room, socket, onLeave }) {
  ...
  return (
    // 🟢 h-[85vh] შეიცვალა min-h-[85vh] h-auto-თი, რომ მობილურზე სქროლვა შეიძლებოდეს და არ გაიჭყლიტოს
    <div className="w-full flex flex-col lg:flex-row gap-6 max-w-7xl mx-auto min-h-[85vh] h-auto pb-10 lg:pb-0">
      
      {/* მარცხენა პანელი (ჩატი და მოთამაშეები) - 🟢 მობილურზე ჩავა დაბლა (order-2) */}
      <div className="w-full lg:w-80 flex flex-col gap-4 order-2 lg:order-1">
        <div className="bg-stone-900/60 backdrop-blur-md border border-white/5 rounded-3xl p-4 md:p-5 shadow-2xl flex flex-col gap-3 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-amber-500/0 via-amber-500/50 to-amber-500/0"></div>
          <h3 className="text-xs font-black text-amber-500 uppercase tracking-widest flex items-center gap-2 mb-1 md:mb-2">
          ...
          {room?.players?.map((p, i) => {
            ...
            return (
              <div key={p.id} className={`relative flex items-center justify-between p-2.5 md:p-3 rounded-2xl border transition-all duration-300 ${isCurrentTurn ? 'bg-amber-500/10 border-amber-500/40 shadow-[0_0_15px_rgba(245,158,11,0.15)]' : 'bg-stone-950/40 border-white/5'}`}>
                ...
                <div className="flex flex-col z-10">
                  <span className={`font-bold text-xs md:text-sm ${p.id === socket.id ? 'text-amber-400' : 'text-stone-200'}`}>
        ...
        <div className="flex-1 bg-stone-900/60 backdrop-blur-md border border-white/5 rounded-3xl flex flex-col shadow-2xl overflow-hidden min-h-[200px] md:min-h-[250px]">
            ...
            {messages.length === 0 ? (
              ...
              messages.map((m, i) => (
                <div key={i} className={`flex flex-col max-w-[85%] ${m.senderId === socket.id ? 'self-end items-end' : 'self-start items-start'}`}>
                  ...
                  <div className={`px-2.5 md:px-3 py-1.5 md:py-2 rounded-2xl text-[10px] md:text-xs font-medium shadow-md ${m.senderId === socket.id ? 'bg-amber-600/90 text-stone-950 rounded-tr-sm' : 'bg-stone-800 text-stone-200 rounded-tl-sm border border-white/5'}`}>
                    {m.text}
                  </div>
                </div>
              ))
          ...
          <form onSubmit={handleSendMessage} className="p-2.5 md:p-3 bg-stone-950/60 border-t border-white/5 flex gap-2">
            <input type="text" value={chatMessage} onChange={e => setChatMessage(e.target.value)} placeholder="დაწერე..." className="flex-1 bg-stone-900 border border-white/10 rounded-xl px-3 py-2 text-[10px] md:text-xs text-stone-200 focus:outline-none focus:border-amber-500/50 transition-all placeholder-stone-600" />
      ...
      {/* მარჯვენა არენა (მაგიდა) - 🟢 მობილურზე ავა ზემოთ (order-1) */}
      <div className="flex-1 bg-stone-900/60 backdrop-blur-xl border border-white/5 rounded-3xl shadow-2xl flex flex-col relative overflow-visible order-1 lg:order-2">
        ...
        <div className="flex items-center justify-between p-3 md:p-4 border-b border-white/5 bg-stone-950/40 rounded-t-3xl">
          <div className="flex items-center gap-2 md:gap-3">
            <span className="text-[10px] md:text-xs font-black text-amber-500 tracking-widest font-mono">ROOM: {room.id}</span>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <span className="text-[10px] md:text-xs font-bold text-stone-400 bg-stone-900/80 px-2.5 md:px-3 py-1.5 rounded-xl border border-white/5 flex items-center gap-1.5 md:gap-2">
              <Trophy size={12} className="text-amber-500"/> {room.targetScore} ქულა
            </span>
          </div>
        </div>
        ...
        {isMyTurn && (
          <div className="w-full h-1 bg-stone-950 overflow-hidden">
            <div className="h-full bg-amber-500 transition-all duration-50" style={{ width: `${timeLeft}%` }} />
          </div>
        )}
        ...
        <div className="flex-1 flex flex-col justify-between p-4 md:p-6 relative">
          ...
          <div className="text-center h-8 md:h-10 relative">
            {isMyTurn ? (
              <div className="inline-flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-amber-500/10 border border-amber-500/30 rounded-full text-amber-400 text-[10px] md:text-xs font-black shadow-[0_0_20px_rgba(245,158,11,0.15)] animate-pulse">
          ...
          <div className="flex-1 flex items-center justify-center relative mt-2 md:mt-4">
            ...
            {room.lastAction && (
              <div className="absolute -top-3 md:-top-6 left-1/2 -translate-x-1/2 bg-stone-900/90 border border-white/10 px-3 md:px-5 py-1.5 md:py-2 rounded-xl md:rounded-2xl shadow-xl z-20 flex flex-col items-center gap-1 md:gap-1.5 animate-in slide-in-from-top-4 duration-300">
                <span className="text-[8px] md:text-[10px] font-black text-stone-400 uppercase tracking-widest whitespace-nowrap">
                  <span className="text-amber-500">{room.lastAction.playerName}</span>-მ {room.lastAction.type === 'CAPTURE' ? 'მოჭრა' : 'დააგდო'}
                </span>
            ...
            {/* 🟢 მაგიდის ზომები და კარტები მორგებულია მობილურზე */}
            <div className="w-full md:w-[85%] max-w-2xl min-h-[11rem] md:min-h-[14rem] py-6 md:py-8 bg-stone-950/40 rounded-[1.5rem] md:rounded-[2rem] border border-white/5 shadow-inner relative flex items-center justify-center px-2 md:px-4 overflow-visible">
          ...
          <div className="flex flex-col items-center gap-3 md:gap-5 mt-4">
            ...
            <div className="flex flex-wrap justify-center items-center gap-2 md:gap-4 bg-stone-950/60 p-2 rounded-2xl border border-white/5 backdrop-blur-md shadow-lg z-10 w-full max-w-[280px] md:max-w-none md:w-auto">
              ...
              <button 
                ...
                className={`w-full md:w-auto px-4 md:px-6 py-2 md:py-2 rounded-xl text-[10px] md:text-xs font-black transition-all shadow-md active:scale-95 ${!isMyTurn || !selectedCardFromHand ? 'bg-stone-800 text-stone-500 cursor-not-allowed' : selectedCardsFromTable.length > 0 ? 'bg-gradient-to-r from-emerald-500 to-emerald-400 text-stone-950 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'bg-gradient-to-r from-amber-600 to-amber-500 text-stone-950 shadow-[0_0_15px_rgba(245,158,11,0.3)]'}`}
              >
                {selectedCardsFromTable.length > 0 ? 'მოჭრა ⚔️' : 'დაგდება 🃏'}
              </button>
            </div>

            {/* 🟢 შენი კარტები მორგებულია მობილურზე */}
            <div className="flex justify-center gap-1.5 md:gap-4 h-28 md:h-40 pt-4 md:pt-8 items-end perspective-1000 overflow-visible w-full">
              {me?.cards?.map((c, i) => {
                ...
                return (
                  <div 
                    ...
                    className={`w-14 h-20 md:w-24 md:h-36 bg-stone-100 rounded-lg md:rounded-xl shadow-2xl flex flex-col justify-between p-1.5 md:p-2.5 select-none transition-all duration-300 ease-out cursor-pointer transform-gpu flex-shrink-0 ${isSelected ? '-translate-y-4 md:-translate-y-8 scale-110 shadow-[0_15px_30px_rgba(0,0,0,0.6)] ring-2 md:ring-4 ring-amber-500 z-20' : 'hover:-translate-y-2 md:hover:-translate-y-4 hover:shadow-[0_15px_30px_rgba(0,0,0,0.5)] z-10'}`}
                  >
                    <span className={`text-xs md:text-lg font-black ${getSuitColor(c.suit)} leading-none`}>{c.rank}</span>
                    <span className={`text-xl md:text-4xl self-center ${getSuitColor(c.suit)} drop-shadow-lg`}>{c.suit}</span>
                    <span className={`text-xs md:text-lg font-black ${getSuitColor(c.suit)} self-end rotate-180 leading-none`}>{c.rank}</span>
                  </div>
                );
              })}
            </div>
        ...
        {/* Overlay მოდალები - ზომები შესწორებულია მობილურისთვის */}
        {room?.roundSummary && (
          <div className="absolute inset-0 bg-stone-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
            <div className="bg-stone-900 border border-amber-500/30 rounded-2xl md:rounded-3xl p-6 md:p-8 max-w-sm md:max-w-md w-full shadow-[0_30px_60px_rgba(0,0,0,0.8)] text-center space-y-4 md:space-y-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <Trophy size={36} className="mx-auto text-amber-500 mb-3 md:mb-4 drop-shadow-[0_0_15px_rgba(245,158,11,0.5)] md:w-[48px] md:h-[48px]" />
                ...
                {room.roundSummary.matchWinner && (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 md:p-4 mb-4 md:mb-6">
                    <p className="text-[10px] md:text-sm font-bold text-amber-500/80 uppercase">გამარჯვებული</p>
                    <p className="text-xl md:text-2xl font-black text-amber-400 mt-1">{room.roundSummary.matchWinner} 🎉</p>
                  </div>
                )}
                ...
                {!room.roundSummary.matchWinner && (
                  <div className="space-y-2 md:space-y-3 bg-stone-950/50 rounded-xl md:rounded-2xl p-3 md:p-4 border border-white/5 text-[10px] md:text-sm font-medium text-stone-300 mb-4 md:mb-6 text-left">
                    <div className="flex justify-between border-b border-white/5 pb-1.5 md:pb-2"><span>ბევრი კარტი:</span> <span className="font-black text-amber-400">{room.roundSummary.cardsWinner}</span></div>
                    <div className="flex justify-between border-b border-white/5 pb-1.5 md:pb-2"><span>ბევრი ჯვარი:</span> <span className="font-black text-amber-400">{room.roundSummary.clubsWinner}</span></div>
                    <div className="flex justify-between border-b border-white/5 pb-1.5 md:pb-2"><span>აგურის 10:</span> <span className="font-black text-amber-400">{room.roundSummary.diamond10Winner}</span></div>
                ...
                <button 
                  ...
                  className="w-full py-3 md:py-4 bg-gradient-to-r from-amber-600 to-amber-500 text-stone-950 rounded-xl text-xs md:text-sm font-black uppercase tracking-widest shadow-lg hover:shadow-[0_0_20px_rgba(245,158,11,0.4)] active:scale-95 transition-all"
                >
